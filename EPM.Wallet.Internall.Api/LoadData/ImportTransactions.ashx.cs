...
namespace WalletInternalApi.LoadData
{
    ...
    /// </summary>
    public class ImportTransactions : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            const string pEpmToRcData = "EPMTORCDATA";
            var xmlText = context.Request.Params[pEpmToRcData];
            Log.Debug($"{pEpmToRcData}={xmlText}");
            ...
            foreach (var accountData in statement.AccountsData)
            {
                ...
                var dFromDate = DateTime.ParseExact(accountData.FromDateUni, "yyyyMMdd", CultureInfo.InvariantCulture).Date;
                var fromDate = dFromDate; //new DateTimeOffset(dFromDate);
                var dToDate = DateTime.ParseExact(accountData.ToDateUni, "yyyyMMdd", CultureInfo.InvariantCulture).Date;
        ...
        private static TransactionDto CreateTransactionDto(TransactionData transactionData, ClientAccountEntity accountEntity)
        {
            ...
            var amount = int.Parse(transactionData.Amount) / 100.00M;
            var amountInCurrency = int.Parse(transactionData.AmountInCurrency) / 100.00M;
